...
def get_xsrf_token():  # brick hill doesnt let you send post requests without the xsrf token. So this initially gets it
    ...
    response = buyingSession.get(
        url="https://www.brick-hill.com/dashboard/",
    )

    if response.url != "https://www.brick-hill.com/login":
        print("Cookie is valid! script will start in 5 seconds!")
        sleep(5)
        ...
        print(
            "Cookie Is Invalid. The script will continue to run but purchases will fail."
        )
        sleep(5)
